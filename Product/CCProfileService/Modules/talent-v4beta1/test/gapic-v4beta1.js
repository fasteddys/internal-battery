...
describe('ProfileServiceClient', () => {
  ...
  describe('createProfile', () => {
    it('invokes createProfile without error', done => {
      ...
      const currentlyPlaced = true;
      const applied = false;
      const previouslyPlaced = true;
      const negativeFeedback = false;
      const expectedResponse = {
        name: name,
        externalId: externalId,
        externalSystem: externalSystem,
...
describe('RecommendationServiceClient', () => {
  describe('recommendProfiles', () => {
    it('invokes recommendProfiles without error', done => {
      ...
      // Mock Grpc layer
      client._innerApiCalls.recommendProfiles = mockSimpleGrpcMethod(
        request,
        expectedResponse
      );

      client.recommendProfiles(request, (err, response) => {
        assert.ifError(err);
        assert.deepStrictEqual(response, expectedResponse);
        done();
      });
    ...
    it('invokes recommendProfiles with error', done => {
      const client = new talentModule.v4beta1.RecommendationServiceClient({
        credentials: {client_email: 'bogus', private_key: 'bogus'},
        projectId: 'bogus',
      });
