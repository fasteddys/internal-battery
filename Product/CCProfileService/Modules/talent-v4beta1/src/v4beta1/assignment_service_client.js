...
class AssignmentServiceClient {
  ...
  constructor(opts) {
    ...
    this._pathTemplates = {
      ...
      assignmentPathTemplate: new gax.PathTemplate(
        'projects/{project}/tenants/{tenant}/profiles/{profile}/assignments/{assignment}'
      ),
    };
   ...
   * };
   * client.createAssignment(request)
   *   .then(responses => {
   ...
   * });
   *
   * const formattedName = client.assignmentPath('[PROJECT]', '[TENANT]', '[PROFILE]', '[ASSIGNMENT]');
   * client.getAssignment({name: formattedName})
   *   .then(responses => {
   *     const response = responses[0];
   *     // doThingsWith(response)
   *   })
   ...
   *
   * const assignment = {};
   * client.updateAssignment({assignment: assignment})
   *   .then(responses => {
   *     const response = responses[0];
   *     // doThingsWith(response)
   *   })
   ...
   *
   * const formattedName = client.assignmentPath('[PROJECT]', '[TENANT]', '[PROFILE]', '[ASSIGNMENT]');
   * client.deleteAssignment({name: formattedName}).catch(err => {
   *   console.error(err);
   * });
   */
   ...
   * const formattedParent = client.profilePath('[PROJECT]', '[TENANT]', '[PROFILE]');
   *
   * client.listAssignments({parent: formattedParent})
   *   .then(responses => {
   ...
   *   }
   * }
   * client.listAssignments({parent: formattedParent}, options)
   *   .then(callback)
   *   .catch(err => {
   *     console.error(err);
   *   });
   ...
   *
   * const formattedParent = client.profilePath('[PROJECT]', '[TENANT]', '[PROFILE]');
   * client.listAssignmentsStream({parent: formattedParent})
   *   .on('data', element => {
   ...
   * @returns {String}
   */
  assignmentPath(project, tenant, profile, assignment) {
    return this._pathTemplates.assignmentPathTemplate.render({
      project: project,
      tenant: tenant,
      profile: profile,
      assignment: assignment,
  ...
  /**
   * Parse the assignmentName from a assignment resource.
   *
   * @param {String} assignmentName
   *   A fully-qualified path representing a assignment resources.
   * @returns {String} - A string representing the project.
   */
  matchProjectFromAssignmentName(assignmentName) {
    return this._pathTemplates.assignmentPathTemplate
      .match(assignmentName)
      .project;
  }
   ...
   * @returns {String} - A string representing the tenant.
   */
  matchTenantFromAssignmentName(assignmentName) {
    return this._pathTemplates.assignmentPathTemplate
      .match(assignmentName)
      .tenant;
  }
   ...
   * @returns {String} - A string representing the profile.
   */
  matchProfileFromAssignmentName(assignmentName) {
    return this._pathTemplates.assignmentPathTemplate
      .match(assignmentName)
      .profile;
  }
