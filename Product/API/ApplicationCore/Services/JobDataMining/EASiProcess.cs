    ...
    public class EASiProcess : BaseProcess, IJobDataMining
    {
        ...
        private async Task<JobPage> CreateJobPageFromHttpRequest(Uri jobPageUri, List<JobPage> existingJobPages)
        {
            ...
            try
            {
                ...
                {
                    _syslog.LogError($"EASiProcess.CreateJobPageFromHttpRequest - maximum retries reached for {uriBuilder.Uri.ToString()}.");
        ...
        public async Task<Tuple<List<JobPage>, long?, int?, int?>> DiscoverJobPages(List<JobPage> existingJobPages)
        {
            ...
            {
                _syslog.LogError($"EASiProcess.DiscoverJobPages - maximum retries reached for {_jobSite.Uri.ToString()}.");
                this._unsuccessfulWebRequests++;
            }
