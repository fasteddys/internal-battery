...
namespace UpDiddyApi.ApplicationCore.Repository
{
    public class ProfileRepository : UpDiddyRepositoryBase<Profile>, IProfileRepository
    {
        ...
        public async Task<Guid> CreateProfile(ProfileDto profileDto)
        {
            ...
            var spParams = new object[] { companyGuid, subscriberGuid, cityGuid, stateGuid, postalGuid, experienceLevelGuid, employmentTypeGuid, contactTypeGuid, firstName, lastName, email, phoneNumber, streetAddress, title, isWillingToTravel, isActiveJobSeeker, isCurrentlyEmployed, isWillingToWorkProBono, currentRate, desiredRate, goals, preferences, profileGuid, validationErrors };

            var rowsAffected = _dbContext.Database.ExecuteSqlCommand(@"EXEC [G2].[System_Create_Profile] @CompanyGuid, @SubscriberGuid, @CityGuid, @StateGuid, @PostalGuid, @ExperienceLevelGuid, @EmploymentTypeGuid, @ContactTypeGuid, @FirstName, @LastName, @Email, @PhoneNumber, @StreetAddress, @Title, @IsWillingToTravel, @IsActiveJobSeeker, @IsCurrentlyEmployed, @IsWillingToWorkProBono, @CurrentRate, @DesiredRate, @Goals, @Preferences, @ProfileGuid OUTPUT, @ValidationErrors OUTPUT", spParams);
        ...
        public async Task UpdateProfileForRecruiter(ProfileDto profileDto, Guid subscriber)
        {
            ...
            var preferences = new SqlParameter { ParameterName = "@Preferences", SqlDbType = SqlDbType.NVarChar, Direction = ParameterDirection.Input, Value = (object)profileDto.Preferences ?? DBNull.Value };
            var validationErrors = new SqlParameter { ParameterName = "@ValidationErrors", SqlDbType = SqlDbType.NVarChar, Size = -1, Direction = ParameterDirection.Output };
            var spParams = new object[] { recruiterSubscriberGuid, profileGuid, companyGuid, subscriberGuid, cityGuid, stateGuid, postalGuid, experienceLevelGuid, employmentTypeGuid, contactTypeGuid, firstName, lastName, email, phoneNumber, streetAddress, title, isWillingToTravel, isActiveJobSeeker, isCurrentlyEmployed, isWillingToWorkProBono, currentRate, desiredRate, goals, preferences, validationErrors };

            var rowsAffected = _dbContext.Database.ExecuteSqlCommand(@"EXEC [G2].[System_Update_Profile] @RecruiterSubscriberGuid, @ProfileGuid, @CompanyGuid, @SubscriberGuid, @CityGuid, @StateGuid, @PostalGuid, @ExperienceLevelGuid, @EmploymentTypeGuid, @ContactTypeGuid, @FirstName, @LastName, @Email, @PhoneNumber, @StreetAddress, @Title, @IsWillingToTravel, @IsActiveJobSeeker, @IsCurrentlyEmployed, @IsWillingToWorkProBono, @CurrentRate, @DesiredRate, @Goals, @Preferences, @ValidationErrors OUTPUT", spParams);
