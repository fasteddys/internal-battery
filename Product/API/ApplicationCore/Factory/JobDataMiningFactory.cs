...
namespace UpDiddyApi.ApplicationCore.Factory
{
    ...
    {
        public static IJobDataMining GetJobDataMiningProcess(JobSite jobSite, IConfiguration config, ILogger logger)
        {
            // todo: remove hard-coded company guids (DI, repository, etc)
            switch (jobSite.Name)
            {
                case "Aerotek":                    
                    return new AerotekProcess(jobSite, logger, new Guid("7E1D8AB0-3440-4773-88B6-2722DA9F2FED"), config);
                case "TEKsystems":
