...
AS
    ...
    SELECT s.AvatarUrl
        ...
        , CAST(CASE WHEN sv.VideoLink IS NOT NULL THEN 1 ELSE 0 END AS BIT) HasVideoInterview
...
BEGIN 
	...
	WHERE SubscriberId IN (SELECT p.SubscriberId FROM @ProfileIndexStatuses g JOIN G2.Profiles p on g.ProfileGuid = p.ProfileGuid WHERE g.IndexStatus in (200,201));
	...
	UPDATE dbo.Subscriber SET
		...
		, AzureIndexModifyDate = GETUTCDATE()
	WHERE SubscriberId IN (SELECT p.SubscriberId FROM @ProfileIndexStatuses g JOIN G2.Profiles p on g.ProfileGuid = p.ProfileGuid WHERE IndexStatus not in (200,201))
END')");
