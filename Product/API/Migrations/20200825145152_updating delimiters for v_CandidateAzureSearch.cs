...
{
    public partial class updatingdelimitersforv_CandidateAzureSearch : Migration
    {
...
AS
    WITH subscriberWorkHistoryWithRownum AS (
        ...
        SELECT s.SubscriberId, STRING_AGG(CAST(sk.SkillName AS NVARCHAR(MAX)), '';'') Skills, dbo.fn_GreatestDate(MAX(s.CreateDate), MAX(ss.ModifyDate), default, default, default) ModifyDate
        FROM Subscriber s
        LEFT JOIN SubscriberSkill ss ON s.SubscriberId = ss.SubscriberId
        LEFT JOIN dbo.Skill sk ON ss.SkillId = sk.SkillId
        WHERE s.IsDeleted = 0 and sk.IsDeleted = 0 and ss.IsDeleted = 0
        ...
        subscriberLanguages AS (			
        SELECT s.SubscriberId, STRING_AGG(CAST(l.LanguageName AS NVARCHAR(MAX)) + ''|'' + pl.ProficiencyLevelName, '';'') SubscriberLanguages, dbo.fn_GreatestDate(MAX(s.CreateDate), MAX(slp.ModifyDate), default, default, default) ModifyDate
        FROM Subscriber s
        LEFT JOIN SubscriberLanguageProficiencies slp ON s.SubscriberId = slp.SubscriberId
        LEFT JOIN ProficiencyLevels pl ON slp.ProficiencyLevelId = pl.ProficiencyLevelId
        LEFT JOIN Languages l on slp.LanguageId = l.LanguageId
