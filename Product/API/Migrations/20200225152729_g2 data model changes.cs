...
{
    public partial class g2datamodelchanges : Migration
    {
...
<description>
Retrieves the most precise geography coordinate based on the provide City, State, and Postal data available.
</description>
<example>
SELECT [dbo].[fn_GetGeographyCoordinate](NULL, NULL, NULL)
SELECT [dbo].[fn_GetGeographyCoordinate](NULL, 1570, NULL)
SELECT [dbo].[fn_GetGeographyCoordinate](45412, 1570, NULL)
SELECT [dbo].[fn_GetGeographyCoordinate](45412, 1570, 857406)
</example>
*/
CREATE FUNCTION [dbo].[fn_GetGeographyCoordinate](@CityId INT, @StateId INT, @PostalId INT)
RETURNS geography
AS
BEGIN
	
	-- Postal is the most specific, so return that first if we have it
	IF @PostalId IS NOT NULL 
	BEGIN
		RETURN (SELECT TOP 1 geography::Point(Latitude, Longitude, 4326) FROM dbo.Postal WHERE PostalId = @PostalId);
	END

	-- City is the less specific than Postal but more specific than State, so return that second if we have it
	IF @CityId IS NOT NULL
	BEGIN
		RETURN (SELECT TOP 1 geography::Point(Latitude, Longitude, 4326) FROM dbo.City c INNER JOIN dbo.Postal p ON c.CityId = p.CityId WHERE c.CityId = @CityId);
	END

	-- State is the least specific but better than nothing, so return this last
	IF @StateId IS NOT NULL
	BEGIN
		RETURN (SELECT TOP 1 geography::Point(Latitude, Longitude, 4326) FROM dbo.[State] s INNER JOIN dbo.City c ON s.StateId = c.StateId INNER JOIN dbo.Postal p ON c.CityId = p.CityId WHERE s.StateId = @StateId);
	END

	RETURN CAST(NULL AS geography);
END')");
            ...
            migrationBuilder.InsertData(
                   ...
                   values: new object[,]
                   {
                        { new Guid("C8614494-7D20-459C-BBD1-DA87F9D4B5F6"), new DateTime(2020, 2, 25), new Guid("00000000-0000-0000-0000-000000000000"), "Public Data", 0, 0, 0 }
                   });
            ...
            migrationBuilder.CreateIndex(
                name: "IX_ProfileSearchLocations_StateId",
                schema: "G2",
                table: "ProfileSearchLocations",
                column: "StateId");
            ...
            migrationBuilder.CreateIndex(
                ...
                unique: true,
                filter: "[CityId] IS NOT NULL AND [PostalId] IS NOT NULL");
            ...
            migrationBuilder.AddForeignKey(
                name: "FK_ProfileSearchLocations_State_StateId",
                schema: "G2",
                table: "ProfileSearchLocations",
                column: "StateId",
...
<description>
Returns TRUE if there are no conflicts between the selected City, State, and Postal records. Otherwise, returns FALSE.
</description>
<example>
SELECT [dbo].[fn_IsValidCityStateAndPostal](1, 2, 3) --returns FALSE
SELECT [dbo].[fn_IsValidCityStateAndPostal](1, NULL, NULL) --returns TRUE
SELECT [dbo].[fn_IsValidCityStateAndPostal](NULL, NULL, NULL) --returns TRUE
SELECT [dbo].[fn_IsValidCityStateAndPostal](45412, 1570, 857406) --returns TRUE
</example>
*/
CREATE FUNCTION [dbo].[fn_IsValidCityStateAndPostal](@CityId INT, @StateId INT, @PostalId INT)
RETURNS BIT
AS
BEGIN
	-- check for conflicts between City and State
	IF @CityId IS NOT NULL AND @StateId IS NOT NULL AND NOT EXISTS(SELECT * FROM dbo.City c INNER JOIN dbo.[State] s ON c.StateId = s.StateId WHERE c.CityId = @CityId AND s.StateId = @StateId)
	BEGIN
		RETURN 0;
	END
	
	-- check for conflicts between City and Postal
	IF @CityId IS NOT NULL AND @PostalId IS NOT NULL AND NOT EXISTS(SELECT * FROM dbo.City c INNER JOIN dbo.Postal p ON c.CityId = p.CityId WHERE c.CityId = @CityId AND p.PostalId = @PostalId)
	BEGIN
		RETURN 0;
	END
	
	-- check for conflicts between State and Postal
	IF @StateId IS NOT NULL AND @PostalId IS NOT NULL AND NOT EXISTS(SELECT * FROM dbo.[State] s INNER JOIN dbo.City c ON s.StateId = c.StateId INNER JOIN dbo.Postal p ON c.CityId = p.CityId WHERE s.StateId = @StateId AND p.PostalId = @PostalId)
	BEGIN
		RETURN 0;
	END
	
	-- assume no conflicts if we got this far
	RETURN 1;
END')");
...
END')");
            ...
            migrationBuilder.Sql(@"ALTER TABLE G2.ProfileSearchLocations ADD CONSTRAINT CK_ProfileSearchLocations_IsValidCityStatePostal CHECK([dbo].[fn_IsValidCityStateAndPostal](CityId, StateId, PostalId) = 1)");

            migrationBuilder.Sql(@"ALTER TABLE G2.Profiles ADD CONSTRAINT CK_Profiles_IsValidCityStatePostal CHECK([dbo].[fn_IsValidCityStateAndPostal](CityId, StateId, PostalId) = 1)");
...
AS
	WITH subscriberWorkHistoryWithRownum AS (
		...
		WHERE s.IsDeleted = 0
		GROUP BY s.SubscriberId, s.FirstName, s.LastName, s.Email, s.PhoneNumber, s.[Address], s.City, t.Code, s.PostalCode, mrswh.Title, mrswh.CompensationTypeName, mrswh.Compensation, s.StateId, s.CreateDate, s.ModifyDate, mrswh.ModifyDate
	)
	, profileTags AS (
	...
	INNER JOIN publicProfiles pp ON cp.SubscriberId = pp.SubscriberId
	INNER JOIN subscriberProfiles sp ON pp.SubscriberId = sp.SubscriberId')");
        }
        ...
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.DeleteData(
                ...
                keyValue: new Guid("C8614494-7D20-459C-BBD1-DA87F9D4B5F6"));

            migrationBuilder.Sql("ALTER TABLE [G2].[Profiles] DROP CONSTRAINT [CK_Profiles_IsValidCityStatePostal]");

            migrationBuilder.Sql("ALTER TABLE [G2].[ProfileSearchLocations] DROP CONSTRAINT [CK_ProfileSearchLocations_IsValidCityStatePostal]");

            migrationBuilder.Sql("DROP FUNCTION [dbo].[fn_IsValidCityStateAndPostal]");

            migrationBuilder.Sql("DROP FUNCTION [dbo].[fn_GetGeographyCoordinate]");
