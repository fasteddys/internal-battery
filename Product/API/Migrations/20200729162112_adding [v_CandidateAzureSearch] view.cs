...
{
    public partial class addingv_CandidateAzureSearchview : Migration
    {
...
*/
CREATE VIEW [B2B].[v_CandidateAzureSearch]
AS
    WITH subscriberWorkHistoryWithRownum AS (
	 ...
	 subscriberTitle AS (
        ...
        SELECT 
		   ...
		   , mrswh.Title
		   , CASE WHEN mrswh.CompensationTypeName = ''Hourly'' THEN mrswh.Compensation ELSE NULL END CurrentRate
		   , ss.Skills 
      ...
      ')");
        ...
        {
            migrationBuilder.Sql(@"DROP VIEW [B2B].[v_CandidateAzureSearch]");
        }
    }
}
